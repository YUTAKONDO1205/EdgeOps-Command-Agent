...
> ハッカソン提出用 Zenn 記事の下書き。本番は Zenn のエディタにコピペし、画像・動画埋め込みを足す。

---

## 1. はじめに

私はこれまで、Sony Spresense と振動・音響センサを用いた異常検知システムを研究してきました。その中で、**異常を検出するだけでは現場の行動は変わらない**と痛感しました。本当に必要なのは「何が起きているのか」「なぜ起きたのか」「誰が次に何をすべきか」まで判断材料を整理することです。
...
📦 ソースコード: https://github.com/YUTAKONDO1205/EdgeOps-Command-Agent

## 2. 解決したい業務課題

設備保全の現場では、以下の情報が **バラバラに** 存在しています。
...
の判断が属人化し、若手作業者は次に何をすべきかが分かりません。

## 3. 作ったもの

**EdgeOps Command Agent** は、点検データを読み取り、

0. **Intake Agent** が入力データの品質を検証する
1. 異常傾向を検出する（**ルールベース**で監査可能に）
2. 原因仮説を尤度付きで出す（**Multi-Agent**）
3. マニュアル根拠を引いてくる（**RAG**）
4. 作業指示を生成する
5. 管理者向け報告書を生成する
6. **Governance Agent** が不確実性を集約し、**人間承認**に回す

までを一気通貫で行います。8 つの Agent がそれぞれ単一責任を持ち、Root Cause Agent が
中央で他 Agent の出力を統合します。
...
![EdgeOps Command Agent Architecture](./architecture.png)

> ↑ 図は `tools/generate_architecture_diagram.py` で PNG 再生成可能。実装と同期した状態を保つため、設計が動いたら必ず再生成して差し替える。
...
設備保全で AI に最終判断を委ねるのは現状非現実的です。**Risk Level は監査可能でなければならない**ため、しきい値による分類は LLM の外側に固定しました。LLM は「数値の解釈」「自然言語化」「原因仮説」「報告書生成」を担います。Zenn 記事のメッセージとして一番伝えたい点はここです。

## 6. プロンプト設計の工夫

すべての Agent 出力は **JSON** に固定し、`response_format={"type": "json_object"}` で構造化しました。プロンプトの設計原則は3つ:
...
特に `ambiguous` は重要です。多くの AI デモは「答えを出して終わり」ですが、本システムは **断片的なスパイク** に対して "確信度低、現場確認推奨" と返します。実務導入時に欲しいのはまさにこの挙動です。

## 8. Human-in-the-loop

すべての提案には「人間承認が必要」フラグが付与され、UI 上で **承認 / 修正依頼 / 却下** を選べます。AI に最終判断を委ねない設計です。

これは飾りではなく、Work Order と Management Report の両方に承認ウィジェットを実装してあります。設備保全に AI を投入する際の合意形成プロセスを **実装レベルで** 想定しているという意思表示です。
...
審査員はこの URL を開くだけで全機能を試せます。Azure OpenAI のクレデンシャルが未設定の場合は **Mock mode** に自動フォールバックする実装にしてあり（`EDGEOPS_USE_MOCK` または endpoint/key の有無で切り替え）、ネットワーク不調時もデモが完走します。
...
- **Microsoft Teams 通知プレビュー** を Command Center に組み込み、Power Automate / Logic Apps による実通知への拡張を意識した形に
...
結果は **20/20 プリセットが全ポリシー合格（総チェック 108/108 件合格）**。詳細マトリクスは `docs/eval_results.md`。
...
異常検知 AI は、**異常を検出するだけでは現場の行動につながりません**。現場で本当に必要なのは、「何が起きているのか」「なぜ起きたのか」「誰が次に何をすべきか」まで判断材料を整理することです。そこで本プロジェクトでは、点検データを異常判断・原因推定・作業指示・報告書に変換する EdgeOps Command Agent を開発しました。

このメッセージが、設備保全に AI を投入しようとしている誰かの参考になれば嬉しいです。
